...
function sortWebListsForCollection(list, collectionName) {
  ...
  if (collectionName === "battalion") {
    ...
    return _.orderBy(listWithFields, ["isMandatory", "active", "position"], ["desc", "desc", "asc"]);
  }
  ...
  if (collectionName === "unit") {
    ...
    return _.orderBy(unitsListWithFields, ["isMandatory", "position", "friendly_id"], ["desc", "asc", "asc"]);
  }
...
function headersToDevice(token, headers) {
  ...
  if (_.has(headers, "x-tc-bundle-identifier") && _.isString(headers["x-tc-bundle-identifier"]) && _.size(headers["x-tc-bundle-identifier"]) > 0) {
