...
var calculateOffsetFromTime = function(time){
...
var fixObjectBooleanKey = function(obj, key, defaultValue){
  if (!_.has(obj, key)) {
...
var fixObjectNumberKey = function(obj, key, defaultValue){
  if(!_.has(obj, key)){
    obj[key] = defaultValue;
    return;
  }
...
var fixObjectStringKey = function(obj, key, defaultValue){
  if(!_.has(obj, key)){
    obj[key] = defaultValue;
    return;
  }
...
var sortWebListsForCollection = function(list, collectionName) {
  if(!_.isArray(list)){
    return list;
  }
  ...
  if(collectionName === 'battalion'){
    ...
    return _.sortByOrder(listWithFields,
      ['isMandatory', 'active', 'position'],
      ['desc', 'desc', 'asc']);
  }
  ...
  if(collectionName === 'unit'){
    ...
    return _.sortByOrder(unitsListWithFields,
      ['isMandatory', 'position', 'friendly_id'],
      ['desc', 'asc', 'asc']);
  }
...
var itemIsTrue = function(item, key) {
...
var hasFeature = function(dept, feature) {
  var value = 0;
...
var isItemValidOnMap = function(item) {
...
var stripSessionFields = function(value, key) {
...
var resolveUser = function(args, callback) {
  var hasSenecaUser = _.isObject(args) &&
    _.isObject(args.req$) &&
    _.isObject(args.req$.seneca) &&
    _.isObject(args.req$.seneca.user);
  ...
  var session = {};
  if (_.isObject(args.req$.seneca.login)) {
...
var resolveLogin = function(args, callback) {
  if (!_.isObject(args) ||
    !_.isObject(args.req$) ||
    !_.isObject(args.req$.seneca) ||
...
var getClosedOrDate = function(){
  var moment = require('moment');
  var nowForClosedDateUnixDate = moment().valueOf()/1000.0;
  var closedOr = [
    {closed_unix_date:0},
    {closed_unix_date:{$gt:nowForClosedDateUnixDate}}
  ];

  return closedOr;
};

var extractInfoFromDevice = function(device){
...
var logUserDevice = function logUserDevice(config, user, session, headers){
  ...
  var postUrl = config.debugUserInfoPostUrl;
  ...
  return request(reqOpts, function(error){
    if(error){
...
var postItem = function postItem(postUrl, item, callback){
  var reqOpts = {
    url: postUrl,
    method: 'POST',
    json: item
...
var configureMomentOpts = function(){
